                ...
                <div class="page-inner">
                    ...
                    <div class="card my-4 p-4">
                        ...
                        @if (!empty($getRecord) && count($getRecord) > 0)
                            ...
                            @foreach ($getRecord as $value)
                                <div class="app-card app-card-settings shadow-sm p-4">
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <span class="px-3 py-1 rounded"
                                                style="background-color:#28a745; color:#fff; font-size:14px; display:inline-block; width:auto;">
                                                <b>{{ $value['name'] }}</b>
                                            </span>
                                                    ...
                                                    @if (!empty($value['exam']))
                                                        @foreach ($value['exam'] as $valueS)
                                                            <tr>
                                                                ...
                                                                <td>{{ \Carbon\Carbon::parse($valueS['exam_date'])->locale('fr')->translatedFormat('l') }}
                                                                </td>
                                                                <td>{{ \Carbon\Carbon::parse($valueS['exam_date'])->format('d-m-Y') }}
                                                                </td>
                                                        ...
                                                        @endforeach
                                                    @else
                                                        {{-- Dan ce cas Quand l’examen existe mais il n’y a aucune matière dedans et elle existe dans la base de donnée --}}
                                                        <tr>
                                                            <td colspan="8" class="text-center text-muted">
                                                                😊 Aucun devoir ou examen pour le moment
                                                            </td>
                                                        </tr>
                                                    @endif
                                                </tbody>
                            ...
                            @endforeach
                            {{-- Dan ce cas Quand aucun examen n’existe du tout dans la base  --}}
                        @else
                            <div class="alert alert-info text-center">
                                😊 Aucun devoir ou examen disponible pour le moment
                            </div>
